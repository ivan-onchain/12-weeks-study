...
contract YourContract {
    function main(address target, address forwarder) public {
        ...
        bytes memory forwardedData = abi.encodeWithSignature("giveTokens(address)", address(this));
        bytes memory data = abi.encodeWithSignature("forward(address,bytes)", target, forwardedData);
        forwarder.call(data);
    }
}
